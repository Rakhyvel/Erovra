	...
	void planeAim() {
		...
		if (smallestPoint.getX() != -1) {
			...
			if (velocity.normalize().dot(position.subVec(patrolPoint).normalize()) > 0.95 && smallestDistance < 73728) {
				if (Main.ticks % 15 == 0 && weight == UnitID.LIGHT) {
					nation.addProjectile(new Bullet(position, nation, position.getTargetVector(smallestPoint.addVector(velocity)), 0.49f));
					...
					nation.addProjectile(new Bullet(position.addPoint(pointA), nation, position.getTargetVector(smallestPoint.addVector(velocity)), 1f));
					nation.addProjectile(new Bullet(position.addPoint(pointB), nation, position.getTargetVector(smallestPoint.addVector(velocity)), 1f));
				}
			}
		} else {
	...
	void acquireTarget() {
		...
		for (int i = 0; i < nation.enemyNation.unitSize(); i++) {
			...
			if (tempUnit.id == UnitID.CITY || tempUnit.id == UnitID.FACTORY || tempUnit.id == UnitID.PORT || tempUnit.id == UnitID.AIRFIELD) {
				Point tempPoint = tempUnit.getPosition();
				int tempDist = (int) position.getDist(tempPoint);
				if (tempUnit.capital) tempDist *= 16;
				if (tempDist < smallestDistance) {
					smallestDistance = tempDist;
					smallestPoint = tempPoint;
				}
