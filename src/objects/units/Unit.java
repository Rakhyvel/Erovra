...
public abstract class Unit {
	 ...
	 */
	public void wander() {
		int smallestDistance = 131072000;
	...
	void clickToMove() {
		...
		} else if (Main.world.selectedUnit.equals(this) && Main.mouse.getMouseLeftDown()) {
	...
	public boolean clearPath(Point point){
		boolean land = true;
		land &= (Map.getArray((int)((point.getX()-position.getX())*.25+position.getX()), (int)((point.getY()-position.getY())*.25+position.getY())) > .5);
		if(!land)
			return land;
		land &= (Map.getArray((int)((point.getX()-position.getX())*.5+position.getX()), (int)((point.getY()-position.getY())*.5+position.getY())) > .5);
		if(!land)
			return land;
		land &= (Map.getArray((int)((point.getX()-position.getX())*.7+position.getX()), (int)((point.getY()-position.getY())*.75+position.getY())) > .5);
		if(!land)
			return land;
		return land;
	}
